...
## Simple Summary
Sometimes there is a need to send more information about a transaction than fits in the provided memo field, for example: KYC info, an invoice, a short note. Such data shouldn't be placed in the [ledger](https://www.zioncoin.org/developers/guides/concepts/ledger.html) because of it's size or private nature. Instead, you should create what we call an attachment. A Zioncoin attachment is simply a JSON document. The `sha-256` hash of this attachment is included as a `memo_hash` in the transaction. The actual attachment document can be sent to the receiver through some other channel, most likely through the receiver's [Auth server](./sep-0003.md).
...
To add the hash to your transaction use the [`TransactionBuilder.addMemo`](http://zioncoin.github.io/js-zioncoin-base/TransactionBuilder.html#addMemo) method.
...
Use [GoZioncoin](https://gozioncoin.org/) application to check if Compliance protocol and attachments were implemented correctly.

## Reference implementations

* [`github.com/zioncoin/go/protocols/compliance` package](https://github.com/zioncoin/go/blob/master/protocols/compliance).
