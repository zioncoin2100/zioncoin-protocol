...
The Compliance Protocol is an additional step after federation. In this step the sending FI contacts the receiving FI to get permission to send the transaction. To do this the receiving FI creates an `AUTH_SERVER` and adds an `AUTH_SERVER` entry to its [`zioncoin.toml`](sep-0001.md).
...
`tx` | string: base64 encoded [xdr.Transaction](https://github.com/zioncoin/zioncoin-core/blob/4961b8bb4a64c68838632c5865389867e9f02840/src/xdr/Zioncoin-transaction.x#L297-L322) | The transaction that the sender would like to send in XDR format. This transaction is unsigned and it's sequence number should be equal `0`.
`attachment` | string | The full text of the attachment. The hash of this attachment is included as a memo in the transaction. The **attachment** field follows the [Zioncoin Attachment Convention](https://www.zioncoin.org/developers/guides/attachment.html) and should contain at least enough information of the sender to allow the receiving FI to do their sanction check.

**sig** is the signature of the data block made by the sending FI. The receiving institution should check that this signature is valid against the public signature key that is posted in the sending FI's [zioncoin.toml](https://www.zioncoin.org/developers/guides/concepts/zioncoin-toml.html) (`SIGNING_KEY` field).
...
Use [GoZioncoin](https://gozioncoin.org/) application to check if Compliance protocol was implemented correctly.

## Reference implementations

* [Reference Compliance server](https://github.com/zioncoin/bridge-server/blob/master/readme_compliance.md) developed by Zioncoin Development Foundation.
* [Compliance structures](https://github.com/zioncoin/go/tree/master/protocols/compliance).
